<script lang="ts">
  ...
  // すべての論文を年の降順で並べる
  $: allPublications = [...pubData].sort((a, b) => b.year - a.year);
  ...
  <div class="container mx-auto px-4 max-w-4xl">
    ...
    {#if visible}
      <div class="content-container" in:fly={{ y: 20, duration: 300, delay: 50 }}>
        <div>
          <div class="space-y-1">
            {#each allPublications as pub, index}
              <div 
                class=""
              ...
              >
                ...
                <!-- 著者名 -->
                <p class="text-gray-700 mb-0.5 text-xs ml-4">
                  {pub.authors}
                </p>
                
                <!-- 掲載情報（イタリック体で区別） -->
                <p class="text-gray-600 italic mb-1 text-xs ml-4">
                  {pub.venue}
                </p>
                
                <!-- 研究キーワード（タグ表示） -->
                <div class="flex flex-wrap gap-1 mb-3 ml-4">
